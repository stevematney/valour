...
var ValidationUnit = (function () {
  ...
  _createClass(ValidationUnit, [{
    ...
    value: function runValidation(value, allValues, name) {
      ...
      var syncRules = this.rules.filter(function (rule) {
        return !rule.isAsync;
      });
      var asyncRules = this.rules.filter(function (rule) {
        return rule.isAsync;
      });
      var getGenerators = function getGenerators(rules) {
        return rules.map(function (rule) {
          return rule.generator;
        });
      };
      var syncGenerators = getGenerators(syncRules);
      var asyncGenerators = getGenerators(asyncRules);
      var getPromises = function getPromises(generators) {
        return generators.map(function (gen) {
          return gen(value, allValues, _this2.messages, name);
        });
      };
      var removeWaiting = function removeWaiting() {
      ...
      var runSync = function runSync() {
        return _Promise.all(getPromises(syncGenerators));
      };
      var runAsync = function runAsync() {
        return _Promise.all(getPromises(asyncGenerators));
      };
      return runSync().then(runAsync).then(passFunc)['catch'](failFunc).then(removeWaiting);
    }
  }, {
    key: 'getState',
    ...
    value: function isEventuallyValidatedBy(func, message) {
      var generator = this.createCustomPromiseGenerator(function (val, allValues, messageList, name, resolve, reject) {
        func(val, allValues, resolve, function () {
          messageList.push((0, _utilFormatValidationMessage2['default'])(message, { name: name }));
          reject();
        });
      });
